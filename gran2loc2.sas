...
/*
proc import datafile="Y:\Documents\ICH_projects\consults\Weiss_Sh\darpa_stats\gran2loc2.csv" 
out=SW.gran2loc2 dbms=dlm  replace;
delimiter='|';
 getnames=yes;
run;
*/;
proc freq data=SW.gran2loc2; table Subject*event; run;

proc freq data=SW.gran2loc2; table Remembered*event /list; by location; 
 ods output List=SW.EventsFreqs_gran2loc2; run;

***** exclude type 3 events and replace NAs; 
data gran2loc2event1; set SW.gran2loc2; if event=1;  
magnitudeN_6=magnitudeN/1000000; 
frequencyN_3=frequencyN/1000;
onsetN_3=onsetN/1000; run;
***** compute the means for event=1; 
proc means data=gran2loc2event1; 
var magnitudeN_6 frequencyN_3 onsetN_3; by location;
output out=SW.event1meansloc2 mean=Mmagn Mfreq Monset; run;

data gran2loc2wo3; set SW.gran2loc2; 
if event < 2.5;  _TYPE_=0;
magnitudeN_6=magnitudeN/1000000; 
frequencyN_3=frequencyN/1000;
onsetN_3=onsetN/1000; run;

data gran2loc2wo3; merge gran2loc2wo3 SW.event1meansloc2; by _TYPE_;
...
***** ***** ***** ***** ***** ***** ***** ***** **** event=3;
data gran2loc2event3; set SW.gran2loc2; if event=3;  
magnitudeN_6=magnitudeN/1000000; _TYPE_=0;
frequencyN_3=frequencyN/1000;
onsetN_3=onsetN/1000; run;

***** compute the means for event=3;
proc means data=gran2loc2event3; 
var magnitudeN_6 frequencyN_3 onsetN_3; by location;
output out=SW.event3meansloc2 mean=Mmagn Mfreq Monset; run;

data gran2loc2only3; merge gran2loc2event3 SW.event3meansloc2; by _TYPE_;
...
run;
 ...
 data gran2loc2_WITH3; set gran2loc2wo3 gran2loc2only3; run;

proc genmod data=gran2loc2_WITH3 descending;
...
REPEATED SUBJECT=Subject/SUBCLUSTER=uniqBL LOGOR=NEST1;
 ods output LSMeans=SW.LSM_w3_gran2loc2 Diffs=SW.DiffLSM_w3_gran2loc2
GEEEmpPEst=SW.Parms_w3_gran2loc2; run;


***** model for events 1 and 2 indicators only;
proc genmod data=gran2loc2wo3 descending;
 class Subject uniqBL event(ref=FIRST);
model Remembered = event/ CL dist=B;
 LSMEANS event/ exp cl diff;  by location;
REPEATED SUBJECT=Subject/SUBCLUSTER=uniqBL LOGOR=NEST1;
 ods output LSMeans=SW.LSMgran2loc2 Diffs=SW.DiffLSMgran2loc2; run;
...
model Remembered = event magnitudeC frequencyC onsetC/ CL dist=B;
 LSMEANS event/ exp cl diff; by location;
REPEATED SUBJECT=Subject/SUBCLUSTER=uniqBL LOGOR=NEST1;
 ods output LSMeans=SW.LSM_wnm_gran2loc2 Diffs=SW.DiffLSM_wnm_gran2loc2
GEEEmpPEst=SW.Parms_wnm_gran2loc2; run;
