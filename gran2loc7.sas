...
/*
proc import datafile="Y:\Documents\ICH_projects\consults\Weiss_Sh\darpa_stats\gran2loc7.csv" 
out=SW.gran2loc7 dbms=dlm  replace;
delimiter='|';
 getnames=yes;
run;
*/;
proc freq data=SW.gran2loc7; table Subject*event; run;

proc freq data=SW.gran2loc7; table Remembered*event /list; by location; 
 ods output List=SW.EventsFreqs_gran2loc7; run;

***** exclude type 3 events and replace NAs; 
data gran2loc7event1; set SW.gran2loc7; if event=1;  
magnitudeN_6=magnitudeN/1000000; 
frequencyN_3=frequencyN/1000;
onsetN_3=onsetN/1000; run;

proc means data=gran2loc7event1; 
var magnitudeN_6 frequencyN_3 onsetN_3; by location;
output out=SW.event1meansloc7 mean=Mmagn Mfreq Monset; run;

***** compute the means for event=1; 
data gran2loc7wo3; set SW.gran2loc7; 
if event < 2.5;  _TYPE_=0;
magnitudeN_6=magnitudeN/1000000; 
frequencyN_3=frequencyN/1000;
onsetN_3=onsetN/1000; run;

data gran2loc7wo3; merge gran2loc7wo3 SW.event1meansloc7; by _TYPE_;
...
proc genmod data=gran2loc7wo3 descending;
...
REPEATED SUBJECT=Subject/SUBCLUSTER=uniqBL LOGOR=NEST1;
 ods output LSMeans=SW.LSMgran2loc7 Diffs=SW.DiffLSMgran2loc7; run;
...
model Remembered = event magnitudeC frequencyC onsetC/ CL dist=B;
 LSMEANS event/ exp cl diff; by location; 
REPEATED SUBJECT=Subject/SUBCLUSTER=uniqBL LOGOR=NEST1;
 ods output LSMeans=SW.LSM_wnm_gran2loc7 Diffs=SW.DiffLSM_wnm_gran2loc7
GEEEmpPEst=SW.Parms_wnm_gran2loc7; run;
