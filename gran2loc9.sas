...
run;
*/;
proc freq data=SW.gran2loc9; table Subject*event; run;

proc freq data=SW.gran2loc9; table Remembered*event /list; by location; 
 ods output List=SW.EventsFreqs_gran2loc9; run;

***** exclude type 3 events and replace NAs; 
data gran2loc9event1; set SW.gran2loc9; if event=1;  
magnitudeN_6=magnitudeN/1000000; 
frequencyN_3=frequencyN/1000;
onsetN_3=onsetN/1000; run;

proc means data=gran2loc9event1; 
var magnitudeN_6 frequencyN_3 onsetN_3; by location;
output out=SW.event1meansloc9 mean=Mmagn Mfreq Monset; run;

***** compute the means for event=1; 
data gran2loc9wo3; set SW.gran2loc9; 
if event < 2.5;  _TYPE_=0;
magnitudeN_6=magnitudeN/1000000; 
frequencyN_3=frequencyN/1000;
onsetN_3=onsetN/1000; run;

data gran2loc9wo3; merge gran2loc9wo3 SW.event1meansloc9; by _TYPE_;
...
proc genmod data=gran2loc9wo3 descending;
...
REPEATED SUBJECT=Subject/SUBCLUSTER=uniqBL LOGOR=NEST1;
 ods output LSMeans=SW.LSMgran2loc9 Diffs=SW.DiffLSMgran2loc9; run;
...
model Remembered = event magnitudeC frequencyC onsetC/ CL dist=B;
 LSMEANS event/ exp cl diff; by location;
REPEATED SUBJECT=Subject/SUBCLUSTER=uniqBL LOGOR=NEST1;
 ods output LSMeans=SW.LSM_wnm_gran2loc9 Diffs=SW.DiffLSM_wnm_gran2loc9
GEEEmpPEst=SW.Parms_wnm_gran2loc9; run;
