...
run;
*/;
proc freq data=SW.gran2loc10; table soz*Remembered*event /list; by location; 
 ods output List=SW.EventsFreqs_gran2loc10_soz; run;

***** model for events 1 and 2 indicators only;
proc genmod data=SW.gran2loc10 descending;
...
REPEATED SUBJECT=Subject/SUBCLUSTER=uniqBL LOGOR=NEST1;
 ods output LSMeans=SW.LSMgran2loc10_soz Diffs=SW.DiffLSMgran2loc10_soz; run;

data SW.DiffLSMgran2loc10_soz; set SW.DiffLSMgran2loc10_soz; if SOZ=_SOZ; run;
/*
proc print data=SW.DiffLSMgran2loc10_soz; run;
